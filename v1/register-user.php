...
try {
    ...
    // Verifica si la columna 'delegacion' existe antes de intentar la inserción
    $stmt = $pdo->prepare("SHOW COLUMNS FROM users LIKE 'delegacion'");
    $stmt->execute();
    $columnExists = $stmt->rowCount() > 0;

    if (!$columnExists) {
        echo json_encode(['message' => "Error: La columna 'delegacion' no existe en la tabla 'users'"]);
        exit;
    }
